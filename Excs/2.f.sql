...
$$
	...
	begin
		for curr_registo_n_proc in (select id_registo as id from registo_n_proc) loop
			...
			from
				registo
			where registo.id = curr_registo_n_proc.id;
			...
			then
				insert into registo_invalido(id_registo) values(curr_registo_n_proc.id);
			else 
				insert into registo_proc(id_registo) values(curr_registo_n_proc.id);
			end if;
		
			-- Remove from non processed 'registos'
			delete from registo_n_proc where registo_n_proc.id = curr_registo_n_proc.id;
